...
import {yatzyHomePage, newYatzyGamePage, newYatzyRoundPage, validateYatzyConfigData,
        validateYatzyRoundData, validateYatzyHighScoreData, yatzyHighScorePage, 
        VALIDATION_ERROR} from "./yatzy-site.js";
...
const PUBLIC_RESOURCES = "node/PublicResources/";

//secture file system access as described on 
//https://nodejs.org/en/knowledge/file-system/security/introduction/
const ROOT_FILE_SYSTEM = process.cwd();
function securePath(userPath) {
    ...
    userPath = PUBLIC_RESOURCES + userPath;

    let p = path.join(ROOT_FILE_SYSTEM, path.normalize(userPath)); 
...
function guessMimeType(fileName) {
    ...
    return (ext2Mime[FILE_EXTENSION] || "text/plain"); 
}
...
function processReq(req,res) {
    ...
    switch (req.method) {
        case "POST": 
            { //route pathname to resource handler function
